...
func main() {
	...
	serverAddr = []*addr{
		...
		{"47.52.79.65:3306", 0, 200 * 1024 * 1024, initWindowsSize},
	}
...
func (hs *httpServer) OnOpened(c gnet.Conn) (out []byte, action gnet.Action) {
	...
	r++
	conn.server = hs.netchan[r%serverNum]
	conn.c = c
...
func ping(server *ServerConn) {
	...
	for {
		...
		case <-time.After(time.Second):
			...
			fmt.Println(server.conn.Write(server.outboundBuffer.Bytes()))
			server.outboundBuffer.Reset()
			<-server.wait
